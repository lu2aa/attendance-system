     ...
     export default function Evaluation() {
       ...
       return (
         <div className="container mx-auto p-6 bg-gray-100 min-h-screen">
           <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center">التقييمات</h1>
           <Link
             href="/evaluation/add"
           ...
           {evaluations.length === 0 ? (
             <p className="text-lg text-gray-600 italic text-center">لا توجد تقييمات. ابدأ بإضافة واحد!</p>
           ) : (
             <div className="max-w-4xl mx-auto overflow-x-auto bg-white shadow-2xl rounded-xl">
               <table className="w-full border-collapse">
                 <thead>
                   <tr className="bg-gray-200">
                     <th className="border-b border-gray-300 p-4 text-left text-sm font-semibold text-gray-700">اسم الموظف</th>
                     <th className="border-b border-gray-300 p-4 text-left text-sm font-semibold text-gray-700">الشهر</th>
                     <th className="border-b border-gray-300 p-4 text-left text-sm font-semibold text-gray-700">الدرجة</th>
                     <th className="border-b border-gray-300 p-4 text-left text-sm font-semibold text-gray-700">التعليقات</th>
                   </tr>
                 </thead>
                 <tbody>
                   {evaluations.map((evaluation) => (
                     <tr key={evaluation.id} className="hover:bg-gray-50 transition duration-200">
                       ...
                       <td className="border-b border-gray-300 p-4 text-sm text-gray-800">{evaluation.evaluation_score || 'غير متوفر'}</td>
                       <td className="border-b border-gray-300 p-4 text-sm text-gray-600">{evaluation.comments || 'غير متوفر'}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
